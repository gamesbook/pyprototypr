...
title = text(font_face="Times New Roman", font_size=8, x=1.4, y=2, align="centre", text=T('{{TITLE}}'))
move = text(font_face="Times New Roman", font_size=12, x=0.6, y=0.5, text=T('{{MOVE}}'))
strength = text(font_face="Times New Roman", font_size=12, x=2.1, y=0.5, text=T('{{STRENGTH}}'))
outline = rectangle(x=0.45, y=0.45, width=2.0, height=2.0, stroke_width=1, stroke=T('{{BORDER}}'), fill=None)
pic = image(T('{{IMAGE}}'), x=0.7, y=0.8, width=1.5, height=1.1)

# "dots" templates
top4 = sequence(
     square(x=0.9, y=2.35, side=0.25, stroke=lbrown, stroke_width=1, fill=T('{{BORDER}}')),
     setting=(1, 4),
     gap_x=0.29)
right3 = sequence(
     square(x=2.3, y=1.0, side=0.25, stroke=lbrown, stroke_width=1, fill=T('{{BORDER}}')),
     setting=(1, 3),
     gap_y=0.29)
low2 = sequence(
     square(x=1.2, y=0.35, side=0.25, stroke=lbrown, stroke_width=1, fill=T('{{BORDER}}')),
     setting=(1, 2),
     gap_x=0.29)
left1 = square(x=0.35, y=1.3, side=0.25, stroke=lbrown, stroke_width=1, fill=T('{{BORDER}}'))
dots4 = group(top4, right3, low2, left1)

top3 = sequence(
     square(x=1.05, y=2.35, side=0.25, stroke=lbrown, stroke_width=1, fill=T('{{BORDER}}')),
     setting=(1, 3),
     gap_x=0.29)
right2 = sequence(
     square(x=2.3, y=1.2, side=0.25, stroke=lbrown, stroke_width=1, fill=T('{{BORDER}}')),
     setting=(1, 2),
     gap_y=0.29)
low1 = square(x=1.3, y=0.35, side=0.25, stroke=lbrown, stroke_width=1, fill=T('{{BORDER}}'))
